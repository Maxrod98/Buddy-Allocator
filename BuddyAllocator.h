...
class BlockHeader{
public:
	bool is_free = true; // size of the block
	char block_size_power; //here I reduced the size of the header by using this variable
...
public:
	...
	if (head == nullptr or b == nullptr)
		return;
		
	//when node to be deleted is in the first node
	if (head == b)
	{
		//element found is the first and only element
	...
	//when not in first node
	BlockHeader * prev = head;

	while (prev->next != nullptr and prev->next != b and prev != prev->next)
		prev = prev->next;
...
private:
	...
	void mergeIteratively(int index);

	BlockHeader* popHeader(int index);
